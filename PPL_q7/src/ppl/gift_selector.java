 ...
 *
 * @see gift_selector 
 * A class creating gift basket in two different ways for a couple
 */
public class gift_selector {
    ...
    */
    ...
    /** Another method for exchanging 
       of gifts between the couples according to  
       question eight where atleast one each type of gift is to be given by the boy .This can be verified from the file "gifting_log.txt" where each couple has exchanged atleast one gift of all the types 
     * @param c array of objects of couple class
     * @param no number of couples
     * @param e  array of objects of essential_gift class
     * @param u  array of objects of utility_gift class
     ...
     * @throws java.io.IOException in case of an error while reading/writing file
    */
   public int[] special_gifting(couple[] c,int no,essential_gift[] e,utility_gift[] u,luxury_gift[] lux) throws IOException{
      int i,j,l,ess,ut; 
     int[] cost_of_gifts = new int[12]; 
      ...
      for(i=0;i<70;i++){
       ...
       for(i=0;i<no;i++){
            ...
            out.write(c[i].b_obj.name+" "+ c[i].g_obj.name+"\n");
                ...
                int budget= c[i].b_obj.budget;
                int cost=0; 
                cost +=(lux[f].price+ e[f].price+ u[f].price);
                budget -=lux[f].price;
                budget -=e[f].price;
                budget -=u[f].price;
                lux[f].assigned=true;e[f].assigned=true;u[f].assigned=true;
               out.write(String.valueOf(lux[f].price)+"  "+String.valueOf(lux[f].value) +" luxury"+"\n");   
                out.write(String.valueOf(e[f].price)+"  "+String.valueOf(e[f].value)+" essential" +"\n");   
               out.write(String.valueOf(u[f].price)+"  "+String.valueOf(u[f].value) +" utility"+"\n");   
                 while(budget>0){
                     l=r.nextInt(70);
                     ut=r.nextInt(70);
                     ess=r.nextInt(70);
                     if(e[ess].assigned=false && budget>=0)   
                     { budget -=e[ess].price;
                      cost += e[ess].price;
                         out.write(String.valueOf(e[ess].price)+"  "+String.valueOf(e[ess].value)+" essential" +"\n"); 
                     }
                     if(u[ut].assigned==false && budget>=0){
                         budget -=u[ut].price;
                         out.write(String.valueOf(u[ut].price)+"  "+String.valueOf(u[ut].value) +" utility"+"\n");
                           cost+=u[ut].price ;
                     }
                      
                         if(lux[l].assigned==false && budget>=0){
                           budget-= lux[l].price;  
                         out.write(String.valueOf(lux[l].price)+"  "+String.valueOf(lux[l].value)+" luxury" +"\n"); 
                          cost += lux[l].price;
                         }
       ...
       /*for(i=0;i<12;i++)
           System.out.println(cost_of_gifts[i]);*/
        out.close();
     return cost_of_gifts;     
       }
