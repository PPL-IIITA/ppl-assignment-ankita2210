...
/**
 *
 * @see test2 class creating gift basket
 */
public class test2 {
    ...
    */
    public int[] gift_basket(couple[] c,int no,essential_gift[] e,utility_gift[] u,luxury_gift[] lux) throws IOException{
        ...
        int[] cost_of_gifts = new int[c.length];
      //  Timestamp timestamp = new Timestamp(System.currentTimeMillis());
        ...
        //    out.write(String.valueOf(timestamp));
          ...
          //        out.write(String.valueOf(timestamp)+" ");       
            ...
            if(c[i].b_obj !=null && "generous".equals(c[i].b_obj.type))
            {
                int cost_g= c[i].g_obj.maint_cost;
                int budget= c[i].b_obj.budget;
                int cost=0;
                while(cost_g>=cost && budget>u[0].value){
                   
                   int util = r.nextInt(70);
                   int lu = r.nextInt(70);
                   if(budget>lux[0].value && lux[lu].assigned==false){
                        budget -= ( lux[lu].price);
           ...
           //      out.write(String.valueOf(timestamp)+" ");       
            ...
            if(c[i].b_obj !=null && "geeks".equals(c[i].b_obj.type)){
                int cost_g= c[i].g_obj.maint_cost;
                int budget= c[i].b_obj.budget;
                int cost=0,lu;
                while(cost_g>=cost){
