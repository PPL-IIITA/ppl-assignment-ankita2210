    ...
    */
     ...
     */
      public void print_k_happy(couple [] c,String happy,int k){
        ...
        for(i=0;i<k;i++){
            System.out.println(c[i].b_obj.name + " "+ c[i].g_obj.name+ " "+ c[i].happy);
...
// instantiating the gifts objects
  ...
  */ 
    ...
    */
    ...
    int[] comp=new int[12]; int t=15;
     boy [] exbf=new boy[12];
        girl [] exgf= new girl[12];
        
        for(i=0;i<12;i++)
        {    exgf[i]=new girl();
                exbf[i]=new boy();
        }  
        /* forming the gifting basket for t(here 15) calender days)
           and checking whose happiness is less than t 
        */
    for(i=0;i<t;i++){
        cost_of_gifts=object.gift_basket(c, 12, ess, util, lux);
        happ=object.happiness(c, 12, cost_of_gifts);
        /*for(int g=0;g<12;g++)
            System.out.println(happ[g]); */
           z=0;
        for(j=0;j<12;j++){
                if(c[j].happy<t){
                   ...
                   exgf[z]=c[j].g_obj;
                   z++;
                   System.out.println(c[j].b_obj.name + " "+ c[j].g_obj.name + "  break up");
                }
         }
        question_4 ob=new question_4();
        ob.breakup(exbf,exgf,z,i+1);
    }
   // comp=object.compatibility(c, 12);  
    String line;
