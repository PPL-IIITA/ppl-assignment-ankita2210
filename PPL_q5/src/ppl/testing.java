...
/**
 * @see testing form couples using the method fnc1() and solves Q5(to print k most happy ) using the method fnc2()
 * using a different method of allocation. First a girl chooses a boy then a boy chooses a girl
 *
 * 
 * 
 */
class instantiate{
    ...
    */
     ...
     */
      ...
      /** This function prints the k most 
...
public class testing {
   ...
   for (int idx = 1; idx <= 48; ++idx){
      ...
      */
      ...
      for(i=0;i<12;i++){
          for(j=i+1;j<12;j++){
              if(g[i].maint_cost>g[j].maint_cost){
                 l=g[i];
                 g[i]=g[j];
                 g[j]=l;
              }
    ...
    System.out.println("The 12-couples formed are : (boy and girl)");
      ...
      for(i=0;i<12;i++){
          if(i%2==0){
              System.out.println("girl chooses a boy");
          if("intelligence_level".equals(g[i].criteria) && g[i].commit== false)
          {   
              int max=-1;
              for(j=0;j<48;j++)
         ...
         else if("attractiveness".equals(g[i].criteria) && g[i].commit== false)
          {   
              int m=-1;
              for(j=0;j<48;j++)
         ...
         else if("richness".equals(g[i].criteria) && g[i].commit== false)
          {   //ob=b[0];
              int m=0;
              for(j=0;j<48;j++)
             ...
             for(q=0;q<48;q++){
                 ...
                 {  b[q].commit=true;
                    g[i].bf_index=q;
                    g[i].commit=true;
                    c[i].b_obj=b[q]; c[i].g_obj=g[i];
                    System.out.println(b[q].name + " "+g[i].name);
                    break;
                 }
                 }
         }
          ...
          {
             System.out.println("boy chooses a girl"); 
             int z,f = 0;
             for(z=0;z<12;z++)
                 if(g[z].commit==false)
                     break;
             girl x=g[z];
             for(j=0;j<12;j++){
                 if(x.attractiveness<g[j].attractiveness && g[j].commit==false  )
                 {  f=j;
                    x=g[j];
                 } 
             }
             ...
             g[f].bf_index=i;
             g[f].commit=true;
             c[i].g_obj=g[f];  c[i].b_obj=b[i];
              System.out.println(b[i].name+" "+ g[f].name);
          }
